...
let song_list = [
  ...
  },
  {
    ...
    artist:
      "Viswanathan-Ramamoorthy,Dharan Kumar,Sathyaprakash,Nithyashree,Emcee D",
  ...
  },
  {
    name: "Nira",
    artist:
      "Nivas K Prasanna,Sid Sriram,Gautham Vasudev Menon,Malvi Sundaresan",
  ...
  },
  {
    ...
    artist:
      "Hiphop Tamizha,Christopher Stanley,Sudharshan Ashok,Srinisha Jayaseelan",
...
function random_color() {
  ...
  document.body.style.background =
    "linear-gradient(to right," + bg_colour1 + "," + bg_colour2 + ")";
}
...
function slider_update() {
  ...
  if (!isNaN(current_track.duration)) {
    current_time.textContent =
      ("0" + Math.floor(current_track.currentTime / 60)).slice(-2) +
      ":" +
      ("0" + Math.floor(current_track.currentTime % 60)).slice(-2);

    total_time.textContent =
      ("0" + Math.floor(current_track.duration / 60)).slice(-2) +
      ":" +
      ("0" + Math.floor(current_track.duration % 60)).slice(-2);
